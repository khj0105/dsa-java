...
class Printer implements Printable, Savable {
	@Override
	public void print() {
...
// Printable + Savable 두 조건을 모두 만족하는 타입만 받을 수 있다
class Device<T extends Printable & Savable> {
	void use(T item) {
		item.print();
		item.save();
	}
...
public class C105_GenericRange2 {
	public static void main(String[] args) {
		// 두 인터페이스를 모두 구현했으므로 가능
		Device<Printer> dv = new Device<>();
		dv.use(new Printer());
		
		// Savable를 구현하지 않았으므로 err
//		Device<Monitor> dv2 = new Device<Monitor>();	// err
	}
}
